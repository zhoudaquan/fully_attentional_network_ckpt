...
CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 python3 -m torch.distributed.launch --nproc_per_node=8 --nnodes=$rank_num \
	--node_rank=$rank_index --master_addr="10.192.33.78" --master_port=3349 \
	main.py  ../imagenet_raw/ --model fan_large_16_p4_hybrid -b 32 --sched cosine --epochs 300 \
	...
	--drop-path .2 --img-size 384 --mixup 0.8 --cutmix 1.0 \
	--smoothing 0.1 \
	--output ../fan_small_12_p16_224/ \
	--amp --model-ema \
	--eval-first \
	--resume ../processed_weights/fan_hybrid_large_in22k_1k_384.pth.tar 
